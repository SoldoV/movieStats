<template>
  <div class="chartElem mt-5">
    ...
    <div v-if="this.$route.params.name === 'all'">
      <div class="chart-row mt-10">
        <div class="mb-5">In the graph bellow we have visualised all directors movie revenue.</div>
...
export default {
  ...
  data () {
    return {
      ...
      scatteredChart: {
        ...
        title: {
          text: 'Movie revenue by directors'
        },
        tooltip: {
          formatter: function () {
            return 'Movie: <b>' + this.point.movie + '<b><br> Director: ' + this.point.name + '<br> Revenue: $' +
                this
  ...
  mounted () {
    db.collection('directors').get().then(querySnapshot => {
      querySnapshot.forEach(doc => {
        this.chartData.push(doc.data())
      })
      this.chartData.forEach(el => {
        if (el.director === this.$route.params.name) {
